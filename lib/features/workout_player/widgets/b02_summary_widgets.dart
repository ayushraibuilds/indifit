...
class B02WorkoutCompletionSuccess extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return CompletionEvidence(launch: launch, onDone: onDone);
  }
}
...
class CompletionEvidence extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final exercises = saved
        ? history!.exercises
              .map(
                (exercise) => HistoryExerciseEvidence.fromHistory(
                  exercise,
        ...
        : (launch?.state.performedExercises ??
              ...
              .map(
                (exercise) => HistoryExerciseEvidence.fromDraft(
                  exercise,
  ...
  static double _draftVolume(List<HistoryExerciseEvidence> exercises) {
    var total = 0.0;
...
class HistoryExerciseEvidence {
  const HistoryExerciseEvidence({
    required this.id,
  ...
  factory HistoryExerciseEvidence.fromHistory(
    B02PerformedExerciseHistory history, {
    required bool showTargets,
  }) {
    return HistoryExerciseEvidence(
      id: history.id,
  ...
  factory HistoryExerciseEvidence.fromDraft(
...
class HistoryGroupEvidence extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    final label = consumerGroupLabel(group.groupType, group.label);
...
class CompletionExerciseEvidence extends StatelessWidget {
  const CompletionExerciseEvidence({super.key, required this.exercise});

  final HistoryExerciseEvidence exercise;
          ...
          child: Column(
            ...
            children: [
              ...
              if (exercise.sets.isNotEmpty) ...[
                const SizedBox(height: 8),
                CompletionPerformedSetList(sets: exercise.sets),
              ],
              if (advanced.isNotEmpty)
                AdvancedEvidenceDisclosure(facts: advanced),
  ...
  List<String> _advancedFacts() {
    ...
    for (final set in exercise.sets) {
      ...
      if (technique.pausedRepPosition != null) {
        facts.add(
          'Paused reps: ${pausedPositionLabel(technique.pausedRepPosition!)}'
          '${technique.pausedRepSeconds == null ? '' : ' · ${technique.pausedRepSeconds} sec'}',
        );
      }
      if (technique.assistanceMode != null) {
        facts.add(
          'Assisted reps: ${assistanceLabel(technique.assistanceMode!)}'
...
class CompletionPerformedSetList extends StatelessWidget {
  const CompletionPerformedSetList({super.key, required this.sets});
...
class AdvancedEvidenceDisclosure extends StatelessWidget {
  const AdvancedEvidenceDisclosure({super.key, required this.facts});
...
String consumerGroupLabel(String type, String? label) {
...
class B02SummaryBody extends StatelessWidget {
  final B02StrengthExecutionLaunch launch;
