  ...
  Future<void> _createV17Indexes() async {
    const statements = [
      ...
      '''CREATE TRIGGER IF NOT EXISTS nutrition_food_fact_preparation_owner_insert
         ...
         BEGIN SELECT RAISE(ABORT, 'Nutrient fact preparation must belong to the same food'); END''',
      ...
      '''CREATE TRIGGER IF NOT EXISTS nutrition_vessel_volume_only_insert
         BEFORE INSERT ON nutrition_personal_vessels
         WHEN NEW.vessel_type IS NOT NULL AND NEW.vessel_type NOT IN ('bowl', 'katori', 'glass', 'cup', 'plate', 'mug', 'spoon')
         BEGIN SELECT RAISE(ABORT, 'Invalid vessel type'); END''',
      '''CREATE TRIGGER IF NOT EXISTS nutrition_vessel_volume_only_update
         BEFORE UPDATE OF vessel_type ON nutrition_personal_vessels
         WHEN NEW.vessel_type IS NOT NULL AND NEW.vessel_type NOT IN ('bowl', 'katori', 'glass', 'cup', 'plate', 'mug', 'spoon')
         BEGIN SELECT RAISE(ABORT, 'Invalid vessel type'); END''',
    ];
