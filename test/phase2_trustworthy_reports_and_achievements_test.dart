...
void main() {
  ...
  group('Phase 2: Trustworthy Reports & Achievements Unit Tests', () {
    ...
    test('2. Logging 1 workout session unlocks "first_workout" and persists timestamp in SQLite', () async {
      ...
      final firstWorkoutBadge = achievements.firstWhere((a) => a.id == 'first_workout');
      final firstPrBadge = achievements.firstWhere((a) => a.id == 'first_pr');
      final heavyMoverBadge = achievements.firstWhere((a) => a.id == 'volume_5000');
      ...
      expect(reloadedStats.unlockedAchievementIds.containsKey('first_workout'), isTrue);
      expect(reloadedStats.unlockedAchievementIds['first_workout'], isNotNull);
    ...
    test('3. Insufficient data (<2 logged days & 0 workouts) returns honest unlock state', () async {
      ...
      expect(report.headline, equals('Log a few more days to unlock your report'));
      expect(report.adherenceScore, equals(0.0));
    });

    test('4. Deleting all records cannot produce non-zero report metrics', () async {
      // Seed data
    ...
    test('5. 7 days of real data produces structured metrics and offline report', () async {
      final mockNow = DateTime(2026, 7, 28, 12, 0);
      final clockedRepo = ProgressStatisticsRepository(db, clock: () => mockNow);
