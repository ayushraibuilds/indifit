...
void main() {
  ...
  group('Data Quality Gap 1 & Schema v18 Tests', () {
    test('AppDatabase initializes with schema version 18', () {
      expect(db.schemaVersion, equals(18));
    });

    test(
      'upsertSeededExercisesFromAsset actually populates the exercise library',
      () async {
        ...
        final benchPress = allExercises.firstWhere(
          (e) => e.name == 'Flat Barbell Bench Press',
        );
    ...
    test(
      'upsertSeededExercisesFromAsset is idempotent (safe to call repeatedly)',
      () async {
        await db.upsertSeededExercisesFromAsset();
  ...
  group('Data Quality Gap 2 — Household Measures Engine Tests', () {
    test(
      'HouseholdMeasure finds units correctly and provides gram equivalents',
      () {
  ...
  group('Data Quality Gap 4 — Meal Templates Repository API Tests', () {
    test(
      'createMealTemplate inserts template and items, getMealTemplates retrieves them',
      () async {
        ...
        expect(
          templates.first.template.name,
          equals('My Usual Indian Breakfast'),
        );
    ...
    test(
      'logMealTemplate batch logs all template items to food logs',
      () async {
        final templateId = await repo.createMealTemplate(
