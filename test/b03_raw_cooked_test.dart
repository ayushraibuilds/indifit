...
void main() {
  ...
  group('B03-09 transformation repository', () {
    ...
    test(
      'Backup-v8 validates target transformation identity before mutation',
      () async {
        final repository = NutritionTransformationRepository(db: db);
        final transformation = _transformation(
          id: 'invalid-target-backup-v1',
          sourceFoodId: 'user-rice',
          sourcePreparationId: 'user-rice-raw',
          targetFoodId: 'user-rice',
          targetPreparationId: 'user-rice-cooked',
        ...
        final provenance = Map<String, dynamic>.from(
          jsonDecode(row['source'] as String) as Map,
        );
        provenance['target_food_id'] = 'missing-target-food';
        row['source'] = jsonEncode(provenance);
        rows[0] = row;
