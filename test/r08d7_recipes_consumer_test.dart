...
void main() {
  ...
  group('R08D.7: Recipe List & Discovery', () {
    testWidgets(
      'Displays published recipes and drafts with clear status and yield',
      (tester) async {
        ...
        await tester.pumpWidget(
          ProviderScope(
            overrides: [
              savedRecipeLogControllerProvider.overrideWith(
                (ref) => controller,
              ),
            ],
    ...
    testWidgets('Empty search shows ProductEmptyState and clear button', (
      tester,
    ) async {
      final controller = SavedRecipeLogController(
        coordinator: Future.value(harness.coordinator),
  ...
  group('R08D.7: Recipe Detail & Ingredient Presentation', () {
    testWidgets(
      'Selecting a recipe displays description, yield, ingredients list, and portion choices',
      (tester) async {
        final controller = SavedRecipeLogController(
          coordinator: Future.value(harness.coordinator),
          userId: harness.userId,
        ...
        final ingredientCatalog = _MappedFoodCatalogRepository(
          db: harness.db,
          registry: harness.registry,
          options: {
            'food::moong_dal': _ingredientOption(
              id: 'food::moong_dal',
              displayName: 'Moong Dal',
            ),
            'food::ghee': _ingredientOption(
              id: 'food::ghee',
              displayName: 'Ghee',
            ),
          },
    ...
    testWidgets('Tapping choose another recipe clears selection', (
      tester,
    ) async {
      final controller = SavedRecipeLogController(
        coordinator: Future.value(harness.coordinator),
  ...
  group('R08D.7: Portion Selection & Nutrition Logging', () {
    testWidgets(
      'Preview displays calculated nutrition facts and confirm button',
      (tester) async {
    ...
    testWidgets('Missing nutrition warning displays acknowledgement checkbox', (
      tester,
    ) async {
      late NutritionRecipeModel recipe;
      late NutritionRecipeVersionModel version;
      ...
      await tester.runAsync(() async {
        ...
        food = (await harness.catalogRepo.getOption('food-partial'))!;
        preview = await harness.coordinator.preview(
          userId: harness.userId,
          recipeId: recipe.id,
      ...
      expect(
        find.text('Missing nutrition stays missing rather than becoming zero.'),
        findsOneWidget,
      );
    });
  ...
  group('R08D.7: Recipe Editor Flow', () {
    testWidgets(
      'Create recipe screen renders cooking language and input fields',
      (tester) async {
        ...
        await tester.pumpWidget(
          ProviderScope(
            overrides: [
              ...
              nutritionRecipeRepositoryProvider.overrideWithValue(
                harness.recipes,
              ),
              nutritionFoodCatalogRepositoryProvider.overrideWith(
                (ref) async => harness.catalogRepo,
              ),
              nutritionTransformationRepositoryProvider.overrideWithValue(
                harness.transformationRepo,
              ),
              nutritionRecipeEditorControllerProvider(
                const NutritionRecipeEditorArgs(),
              ).overrideWith((ref) => editorController),
            ],
            child: const MaterialApp(home: NutritionRecipeEditorScreen()),
          ),
        );
        await tester.pump();
        ...
        expect(
          find.text('Cooking notes & instructions (optional)'),
          findsOneWidget,
        );
  ...
  group('R08D.7: Accessibility & Responsiveness', () {
    testWidgets(
      'SavedRecipeLogScreen renders cleanly at 320pt with 2x text in dark theme',
      (tester) async {
        ...
        controller.state = controller.state.copyWith(
          ...
          recipes: const [
            NutritionRecipeModel(
              ...
              description:
                  'Long description to test narrow text scaling behavior without flex overflow',
              lifecycle: NutritionRecipeLifecycle.active,
    ...
    testWidgets(
      'NutritionRecipeEditorScreen renders cleanly at 320pt with 2x text in light theme',
      (tester) async {
        tester.view.physicalSize = const Size(320, 568);
        tester.view.devicePixelRatio = 1;
        addTearDown(tester.view.reset);
...
class _Harness {
  ...
  Future<void> insertFood({
    ...
    final facts =
        <
          String,
          ({double amount, double? lower, double? upper, String status})
        >{
          'energy': (
            amount: 100,
...
class _MappedFoodCatalogRepository extends NutritionFoodCatalogRepository {
  _MappedFoodCatalogRepository({
    required super.db,
    required super.registry,
    required this.options,
  });

  final Map<String, NutritionFoodOption> options;
...
NutritionFoodOption _ingredientOption({
  required String id,
  required String displayName,
}) => NutritionFoodOption(
  id: id,
  displayName: displayName,
  baseQuantity: Quantity.fromDecimal(amount: '100', unit: QuantityUnit.gram),
  facts: const {},
  sourceType: 'fixture',
