...
void main() {
  ...
  group('R08G.8 Visual & Responsive Polish', () {
    testWidgets(
      'Settings landing renders without overflow on narrow width (320pt)',
      (tester) async {
    ...
    testWidgets(
      'Settings landing renders without overflow at 2.0x text scale in dark mode',
      (tester) async {
    ...
    testWidgets(
      'Sub-screens render cleanly at narrow 320pt and 2.0x text scale',
      (tester) async {
        tester.view.physicalSize = const Size(320, 640);
        tester.view.devicePixelRatio = 1.0;
        addTearDown(tester.view.resetPhysicalSize);
      ...
      (tester) async {
        ...
        expect(
          find.bySemanticsLabel(
            RegExp('Dietary needs & preferences', caseSensitive: false),
          ),
          findsAtLeastNWidgets(1),
        );
        await tester.scrollUntilVisible(
          find.text('Health integration'),
          200,
          scrollable: find.byType(Scrollable).first,
        );
        expect(
          find.bySemanticsLabel(
            RegExp('Health integration', caseSensitive: false),
          ),
          findsAtLeastNWidgets(1),
        );
        await tester.scrollUntilVisible(
          find.text('Notifications & reminders'),
          200,
          scrollable: find.byType(Scrollable).first,
        );
        expect(
          find.bySemanticsLabel(
            RegExp('Notifications & reminders', caseSensitive: false),
          ),
          findsAtLeastNWidgets(1),
        );
        semantics.dispose();
