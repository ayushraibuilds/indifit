...
void main() {
  ...
  group('Phase 1: Shared Data Foundation Unit & Integration Tests', () {
    ...
    test('1. Schema v14 initializes with new tables and extended user profile columns', () async {
    ...
    test('2. BackupData Version 5 round-trips all new tables and extended profile data', () async {
      // Seed user profile
      await db.into(db.userProfiles).insert(
            UserProfilesCompanion.insert(
      ...
      expect(parsed.achievementUnlocks.first.achievementId, equals('century_club'));

      // Perform atomic restore to a fresh database
      final freshDb = AppDatabase.memory();
      await parsed.restoreToDatabase(freshDb);

      final restoredProfiles = await freshDb.select(freshDb.userProfiles).get();
      final restoredHydrations = await freshDb.select(freshDb.dailyHydrations).get();
      final restoredUnlocks = await freshDb.select(freshDb.achievementUnlocks).get();
    ...
    test('3. BackupData parses legacy Version 4 backups with default fallbacks', () async {
      final legacyV4Json = {
        'version': 4,
    ...
    test('4. ProgressStatisticsRepository calculates 7-day metrics with clock injection', () async {
    ...
    test('5. AdherenceBreakdown dynamically reweights applicable metrics', () async {
