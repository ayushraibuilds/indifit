...
void main() {
  ...
  test(
    'duplicating a published version records cross-recipe provenance without ancestry drift',
    () async {
      final db = AppDatabase.memory();
      addTearDown(db.close);
      await _seedFoods(db, const ['food-a']);
      final repository = NutritionRecipeRepository(db: db);
      final original = await repository.createRecipe(
        userId: 'recipe-user',
        ...
        ingredients: [_ingredient('original-line', 'food-a', '100')],
      );
      await repository.publishDraft(
        recipeId: original.recipe.id,
        draftVersionId: original.version.id,
      );

      final duplicate = await repository.duplicatePublishedVersion(
        sourceVersionId: original.version.id,
        userId: 'recipe-user',
        recipeId: 'duplicated-recipe',
        versionId: 'duplicated-v1',
      );
