...
void main() {
  ...
  group('Phase 3: Personalisation & Meal Quality Unit Tests', () {
    ...
    test(
      '2. Vegan meal plan fallback generates zero dairy, egg, or meat across all 7 days',
      () async {
        ...
        for (final day in result.days) {
          ...
          for (final kw in forbiddenKeywords) {
            ...
            final textWithoutNutButter = allText.replaceAll(
              RegExp(
                r'\b(peanut|almond|cashew)\s+butter\b',
                caseSensitive: false,
              ),
              '',
            );
            expect(
              ...
              isFalse,
              reason:
                  'Day ${day['day']} vegan plan contained forbidden non-vegan keyword "$kw"',
            );
          }
        }
        ...
        for (final kw in forbiddenKeywords) {
          final regex = RegExp(r'\b' + kw + r's?\b', caseSensitive: false);
          final groceryWithoutNutButter = groceryText.replaceAll(
            RegExp(
              r'\b(peanut|almond|cashew)\s+butter\b',
              caseSensitive: false,
            ),
    ...
    test(
      '3. Non-Vegetarian meal plan fallback incorporates non-veg protein options',
      () async {
        final result = await mealPlanService.generateMealPlan(
          calorieGoal: 2000,
        ...
        expect(
          ...
          isTrue,
          reason:
              'Non-veg meal plan should contain chicken, egg, fish, or mutton.',
        );
      },
    );

    test(
      '4. Portion scaling produces calories strictly proportional to target calories (3000 vs 2000 kcal)',
      () async {
        ...
        final mondayLunch2000 =
            result2000.days.firstWhere((d) => d['day'] == 'Monday')['lunch']
                as String;
        final mondayLunch3000 =
            result3000.days.firstWhere((d) => d['day'] == 'Monday')['lunch']
                as String;
