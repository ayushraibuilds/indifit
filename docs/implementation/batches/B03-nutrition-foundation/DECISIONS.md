...
- **Invariants:** Personal calibration overrides only generic volume in the user scope. Deletion/archive never rewrites snapshots. Approximate values are labelled approximate; unknown conversion is visible. Existing global `HouseholdMeasure` equivalents remain compatibility behavior, not B03 truth.
