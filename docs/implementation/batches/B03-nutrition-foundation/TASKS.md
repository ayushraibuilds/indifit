...
Status: planning only. The architecture gate is **Passed with automatically
accepted defaults**, but no B03 feature implementation or branch is authorized
until the dependencies and review points below are satisfied. `SOL-GATE
REQUIRED` means the task has a mandatory Sol High review before its durable
consumer or downstream task can land.
...
- **Exact scope:** The 24 proposal tables plus the D15 amendments and the required `nutrition_snapshot_constraint_result_evidence` child relation (25 physical tables after amendment): no recipe kind in `nutrition_foods`, sole legacy mapping bridge, direct-food recipe ingredient contract, volume-only vessel calibration, typed constraint evidence relation, complete snapshot columns/rows, and no nested recipe implementation.
...
- **Acceptance criteria:** Fresh v17 and real v16→v17 databases have valid FKs/indexes; failure injection rolls back DDL/data; legacy logical rows and active drafts/unsynced markers survive; no B03 table can be populated with an invalid dimension/status/relationship.
