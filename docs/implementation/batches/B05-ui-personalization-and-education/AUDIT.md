...
| Top-20 media and muscle diagrams | `assets/data/exercises.json`, exercise cues/mistakes, B02 mappings; no media assets or diagram registry | B05-01, B05-07, B05-08 | Required but gated on exact selection, license/source and packaging approval. Text/list fallback is accessible behavior, not a substitute for the required approved pack. |
...
| Is schema/backup change justified? | `AppDatabase.schemaVersion == 18`; `BackupV9Data.currentVersion == 9`; transactional restore path | Yes. Layout, content completion, downloaded-pack metadata, and a playlist choice are user-owned portable state. Media binaries, local paths, provider credentials, and raw provider payloads must not be backed up. |
...
| `downloaded_media_manifests` | approved pack/asset IDs, version, checksum, availability, updated-at | Reconciles optional local packs after restore without copying their bytes | Binary clips, local path, tokens, analytics/telemetry payload |
...
data. The database holds user state and pack availability only.

## Risks and non-negotiable boundaries

1. **Rights and packaging are a blocking product input.** The exact top-20
   stable IDs, rights, attribution, approved sources, distribution constraints,
   max package/download budget, diagrams and provider allowlist must be
   approved before B05-08. Missing approval keeps B05 incomplete; it does not
   authorize remote scraping or placeholder copyright material.
...
- B05-01 is the sole schema/backup/content/asset-contract writer and must land
  before any B05 persisted state is shown.
...
- B05-04 through B05-09 use declared file ownership and at most two
  ...
  serialized.
- B05-08 is mandatory but license-gated. Its absence is a visible B05 blocker,
  not a post-launch reclassification.
- B05-10 and B05-11 run only on a clean integrated head and distinguish
  evidenced defects from unavailable credentials/devices or historical
  administrative gaps.
