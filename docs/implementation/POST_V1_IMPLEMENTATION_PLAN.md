...
**Status:** Complete (CLOUD-01A threat model & envelope encryption, CLOUD-01B immutable upload slice & FastAPI endpoints, and CLOUD-01C transactional restore & Settings Data Management UI integration frozen and verified across automated test suites).
...
3. **CLOUD-01C restore and operations:** list eligible snapshots, verify before
   ...
   support back-up-now, schedule/retry, retention pruning, and remote deletion. (Complete)
...
- **Status:** SYNC-01A (Specification & HLC engine) and SYNC-01B (Vertical Slice: SyncService, SyncApiClient, FastAPI /v1/sync endpoints, and multi-device convergence tests) Complete.
...
- **Status:** CATALOG-01A (Specification & models), CATALOG-01B (FoodCatalogService & ReviewSheet), and CATALOG-01C (Barcode camera/input flow with local cache precedence and review integration frozen at `test/pv1_catalog01c_barcode_scan_test.dart`) Complete.
...
| 14.1 | PV1-ENG-05D | Mechanically split food screens and diary presentation | P1 | PV1-ENG-02 |
| *Status:* | | **Complete** — Split `food_search_screen.dart` (1,598 → 646 lines) and `nutrition_diary_screen.dart` (1,232 → 522 lines) into widget and sub-screen components; screens re-export parts for routers and test consumers. Verified 0 analyzer issues and R09-D release gate | | |
| 14.2 | PV1-ENG-05E | Mechanically split routine authoring presentation | P1 | PV1-ENG-02 |
| *Status:* | | **Complete** — Split `b02_routine_authoring_screen.dart` (1,677 → 806 lines) into editor forms, review helpers, and authoring widgets; preserved all state controllers. Verified 0 analyzer issues and R09-D release gate | | |
...
| *Status:* | | **Complete** — Formal specification created in `docs/implementation/post-v1/PROD02A_APPROVED_EXERCISE_MEDIA_COMPLETION.md`. Defined clean public-repo invariant (zero WebPs in git via `.gitignore:61` and `validate_r08_0_3_public_repo.dart`), pinned RepDB commit `045845b...`, exact counts (30 sets, 59 files, 120 bindings, 5 rejects), DPR-to-cacheWidth decoding limits, and non-visual accessibility semantics. | | |
| 18.1 | PV1-PROD-02B | Implement approved exercise media completion and interactive pose inspection | P1 | PV1-PROD-02A |
| *Status:* | | **Complete** — Fixed download pipeline in `tool/acquire_r08_repdb_assets.dart` and sentinel in `tool/validate_r08_third_party_assets.dart`, acquired all 59 production WebPs (1,008,360 bytes), added interactive Start/Peak pose switcher via `SegmentedButton` and movement technique disclosure caption to `ExerciseDetailsSheet`, added DPR-bounded `cacheWidth` decoding to `ExerciseLibraryScreen` list thumbnails, and verified 9/9 media tests and 40/40 targeted tests passing. | | |
